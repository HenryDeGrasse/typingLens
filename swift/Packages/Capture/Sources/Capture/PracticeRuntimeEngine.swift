...
final class PracticeRuntimeEngine {
    // Cap input event retention so a runaway long block can't grow unboundedly.
    // Practical 55s drill blocks produce ~1k events; 5k leaves plenty of headroom.
    private static let maxRetainedInputEventsPerBlock = 5_000
    ...
    func handleCharacter(_ rawCharacter: String) {
        mutateActiveBlock { block in
            guard let normalizedCharacter = Self.normalizedCharacter(from: rawCharacter),
                  !block.prompts.isEmpty,
                  let activePrompt = block.activePrompt?.text.lowercased() else {
                return
            }

            let promptCharacters = Array(activePrompt)
            guard !promptCharacters.isEmpty,
                  block.typedText.count < promptCharacters.count else { return }

            let nextIndex = block.typedText.count
            Self.appendBoundedInputEvent(
                to: &block.inputEvents,
    ...
    private static func appendBoundedInputEvent(
        to events: inout [PracticeInputEvent],
        event: PracticeInputEvent
    ) {
        events.append(event)
        if events.count > maxRetainedInputEventsPerBlock {
            // Drop the oldest quarter to amortize cost; analyzers are sample-based and tolerate this.
            let dropCount = maxRetainedInputEventsPerBlock / 4
